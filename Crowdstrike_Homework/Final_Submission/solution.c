#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <assert.h>
#include <sys/socket.h>
#include <netdb.h>
#include <sys/types.h>
#include <unistd.h>
#include "http_parser.c"

#define URLS_FNAME "URLs.txt"
#define MAX_URL_SIZE 1028
#define MAX_FNAME 64
#define MAX_LNAME 64
#define MAX_AGE_LEN 4
#define MAX_AGE 128
#define CSV_HEADER "fname, lname, age"
#define HEADER_LEN 18
#define DEFAULT_PORT 8080
#define PORT_LEN 6
#define DEFAULT_SERVERNAME "localhost"
#define BUFF_LEN 1024


typedef struct Tfullname {
    char *fname;
    char *lname;
} Tfullname;


//globals
unsigned int max_csv_line = MAX_FNAME + MAX_LNAME + MAX_AGE_LEN + 2;
unsigned int Age_counter[MAX_AGE];
Tfullname Age_dict[MAX_AGE];


//functions
int my_stoi(unsigned int *n, char *s) {
    unsigned int x, i;
    x = i = 0;

    while (s[i] != 0) {
        if (s[i] > 47 && s[i] < 58) {
            x = x*10 + s[i] - 48;
        }
        else {
            return 1;
        }
        i++;
    }
    *n = x;
    return 0;
}

//converts unsigned integers to strings
void my_itos(unsigned int n, char* s, unsigned int max_len) {
    if (n == 0) {
        s[0] = '0';
        s[1] = 0;
    }
    else {
        int i;
        char *tmp = (char *)malloc(max_len);
        assert(tmp);
        tmp[max_len-1] = 0;

        for (i=max_len-2; i>=0; i--) {
            if (n == 0) {
                break;
            }
            tmp[i] = n % 10 + 48;
            n /= 10;
        }

        strcpy(s, tmp + 1 + i);
        free(tmp);
    }
}

//like strncpy but it doesn't care about null termintors at all
void my_strncpy(char *dest, char *src, unsigned int n) {
    for (unsigned int i = 0; i < n; i++) {
        dest[i] = src[i];
    }
}

//makes use of a parser from the Joyent/HTTP parser lib
//also makes use of https://titanwolf.org/Network/Articles/Article?AID=95c58768-593b-478c-a2e2-edb90a3fd43f
//all code was reworked with the intention of simply parsing a URL
int connect_to_url(char* url) {
    unsigned int port_n;
    int r;
    struct http_parser_url u;
    char* servername;
    char port_s[PORT_LEN];


    r = http_parser_parse_url(url, strlen(url), 0, &u);
    if (r) {
        return -1;
    }

    if (u.field_set & 4) {
        port_n = (unsigned int)u.port;
    }
    else {
        port_n = DEFAULT_PORT;
    }
    my_itos(port_n, port_s, PORT_LEN);

    if (u.field_set & 2) {
        servername = (char*)malloc(u.field_data[UF_HOST].len+1);
        strncpy(servername, url+u.field_data[UF_HOST].off, u.field_data[UF_HOST].len);
        servername[u.field_data[UF_HOST].len] = 0;
    }
    else {
        servername = (char*)malloc(10);
        servername = DEFAULT_SERVERNAME;
    }

    //prepare hints for getaddrinfo
    struct addrinfo hints;
    memset(&hints, 0, sizeof(hints));
    hints.ai_family = AF_INET;
    hints.ai_socktype = SOCK_STREAM;

    //get server address info
    struct addrinfo *serverinfo;
    int res = getaddrinfo(servername, port_s, &hints, &serverinfo);
    if (res != 0) {
        free(servername);
        return -1;
    }
    //create socket
    int sock_fd = socket(serverinfo->ai_family, serverinfo->ai_socktype, serverinfo->ai_protocol);
    if (sock_fd == -1) {
        free(servername);
        freeaddrinfo(serverinfo);
        return -1;
    }

    //connect to the server
    res = connect(sock_fd, serverinfo->ai_addr, serverinfo->ai_addrlen);
    if (res == -1) {
        free(servername);
        freeaddrinfo(serverinfo);
        return -1;
    }
    
    free(servername);
    freeaddrinfo(serverinfo);

    return sock_fd;  
}

//returns the number of bytes read
//this is only less than len if we reached the EOF of server disconnected
int recvall(int s, char *buff, unsigned int len) {
    unsigned int total = 0;
    unsigned int bytesleft = len;
    int n;

    do {
        n = read(s, buff + total, bytesleft);
        if (n == 0) return total;

        total += n;
        bytesleft -= n;
    } while (bytesleft);
    return total;
}

//returns 0 if everything worked
//return 1 if it reached the end of file and read no data
//returns 2 if string was too long (it then continues reading but doesn't add to the buffer)
int readline(char *s, unsigned int size, FILE *stream) {
    char c;
    unsigned int i;
    for (i = 0; i < size - 1; i++) {
        c = (char)fgetc(stream);
        if (c == '\n' || c == EOF) {
            break;
        }
        *s = c;
        s++;
    }
    *s = 0;

    //check for various errors
    //We found an end of file without reading any data
    if (i == 0 && c == EOF) {
        return 1;
    }

    //we never found \n or EOF, so too long
    //Here, keep going until we reach the end of the file or a newline but don't add anything else to the buffer
    if (i == size - 1) {
        while (c != '\n' && c != EOF) {
            c = (char)fgetc(stream);
        }
        return 2;
    }

    //everything worked fine
    return 0;
}

//moves a line over from the main buffer to a seperate csv_line
//returns the number of bytes moved
unsigned int moveline(char *dest, unsigned int max_size, char *src) {
    char c;
    unsigned int i;
    for (i = 0; i < max_size; i++) {
        c = src[i];
        if (c == '\n' || c == EOF) {
            break;
        }
        *dest = c;
        dest++;
    }
    *dest = '\n';

    return i + 1;
}

//returns -1 on error and 0 on success
int unpack_row(char *fname, char *lname, char *age_arr, char *csv_line) {
    char c;
    unsigned int i, j;

    //Fill in the firstname
    for (i = 0; i < MAX_FNAME - 1; i++) {
        c = csv_line[i];
        if (c == ',' || c == '\n') {
            break;
        }
        fname[i] = c;
    }
    fname[i] = 0;
    if (c == '\n' || i == MAX_FNAME - 1) {
        return -1;
    }
    i += 2;

    //fill in the last name
    for (j = 0; j < MAX_LNAME - 1; j++) {
        c = csv_line[i];
        if (c == ',' || c == '\n') {
            break;
        }
        lname[j] = c;
        i++;
    }
    lname[j] = 0;
    if (c == '\n' || j == MAX_LNAME - 1) {
        return -1;
    }
    i += 2;

    //fill in the age
    for (j = 0; j < MAX_AGE_LEN - 1; j++) {
        c = csv_line[i];
        if (c == ',' || c == '\n') {
            break;
        }
        age_arr[j] = c;
        i++;
    }
    age_arr[j] = 0;
    if (c == ',' || j == MAX_AGE_LEN - 1) {
        return -1;
    }

    return 0;
}

int unpack_file(int s, char *buff) {
    char *csv_line = (char*)malloc(max_csv_line);
    char fname[MAX_FNAME];
    char lname[MAX_LNAME];
    char age_arr[MAX_AGE_LEN];
    unsigned int age;
    int last_newline, i, j, r, n, flag = 0;
    int start = 0;
    int buff_remaining = BUFF_LEN;

    while (1) {
        //Try to fill up the buffer
        n = recvall(s, buff + start, buff_remaining);

        if (n == 0) {
            break;
        }

        //Count back to find the last newline
        for (i = start + n - 1; i >= start; i--) {
            if (buff[i] == '\n') {
                last_newline = i;
                break;
            }
        }

        //we never found the newline - drop the whole buffer and refill
        if (i < start) {
            start = 0;
            buff_remaining = BUFF_LEN;
        }

        //we found the newline - move and read all of those lines
        else {
            //j is going to track how far moveline has gone
            j = 0;
            do {
                j += moveline(csv_line, last_newline - j + 1, buff + j);
                
                r = unpack_row(fname, lname, age_arr, csv_line);
                
                //Unpack_row worked
                if (r == 0) {
                    r = my_stoi(&age, age_arr);

                    //my_stoi worked
                    if (r == 0 && age < MAX_AGE) {
                        //update the globals
                        Age_counter[age]++;
                        if (Age_dict[age].fname == NULL) {
                            char *dict_fname = (char*)malloc(MAX_FNAME);
                            strcpy(dict_fname, fname);
                            Age_dict[age].fname = dict_fname;
                            char *dict_lname = (char*)malloc(MAX_LNAME);
                            strcpy(dict_lname, lname);
                            Age_dict[age].lname = dict_lname;
                        }
                    }
                }

                if (r != 0) flag = -1;

            } while (j < last_newline);

            //copy the remaining data to the front and prepare to recieve again
            start = start + n - last_newline - 1;
            buff_remaining = BUFF_LEN - start;
            my_strncpy(buff, buff + last_newline + 1, start);
        }
    }

    free(csv_line);
    return flag;
}

int main() {
    //initialize things
    FILE *URL_file;
    char curr_URL[MAX_URL_SIZE];
    char *buff = (char*)malloc(BUFF_LEN);
    int sock_fd, r;
    unsigned long n, s;
    unsigned int i, median, m_age;
    double mean;
    float median_f;

    //Open the URL file
    URL_file = fopen(URLS_FNAME, "r");
    if (URL_file == NULL) {
        printf("could not find %s\n", URLS_FNAME);
        return 0;
    }

    while(1) {
        //read a URL
        r = readline(curr_URL, MAX_URL_SIZE, URL_file);

        if (r == 1) {
            break;
        }

        //connect to the URL
        sock_fd = connect_to_url(curr_URL);

        //check the basics of this socket
        if (sock_fd != -1) {
            recvall(sock_fd, buff, HEADER_LEN);

            //replace the newline terminator with a null terminator
            buff[HEADER_LEN - 1] = 0;

            r = strncmp(buff, CSV_HEADER, HEADER_LEN);
            if (r == 0) {

                //here we will unpack the rest of the data
                r = unpack_file(sock_fd, buff);
            }

            //problem unpacking the CSV file
            if (r != 0) {
                printf("Problem unpacking %s\n", curr_URL);
            }

            //disconnect from the server
            close(sock_fd);
        }

        else {
            printf("Problem connecting to %s\n", curr_URL);
        }
    }

    //free things
    free(buff);
    assert(fclose(URL_file) == 0);

    //analyze the globals
    n = s = 0;
    for (i = 0; i < MAX_AGE; i++) {
        n += Age_counter[i];
        s += Age_counter[i] * i;
    }
    
    mean = (double)s / (double)n;
    printf("mean is %f\n", mean);

    //odd case median
    if (n % 2) {
        int x = n / 2 + 1;
        for (i = 0; i < MAX_AGE; i++) {
            x -= Age_counter[i];
            if (x < 1) {
                m_age = i;
                break;
            }
        }
        printf("median is %d\n", m_age);
    }

    //even case median
    else{
        int x = n / 2;
        int y = x + 1;
        unsigned int m1, m2;
        //start m1 at an impossible age
        m1 = MAX_AGE;
        for (i = 0; i < MAX_AGE; i++) {
            x -= Age_counter[i];
            y -= Age_counter[i];
            if (x < 1 && m1 == MAX_AGE) {
                m1 = i;
            }
            if (y < 1) {
                m2 = i;
                break;
            }
        }

        m_age = m1;
        if (m1 == m2) {
            printf("median is %d\n", m_age);
        }
        else if ((m2 - m1) % 2) {
            median_f = (float)(m2 + m1) / 2;
            printf("median is %f\n", median_f);
        }
        else {
            median = (m2 + m1) / 2;
            printf("median is %d\n", median);
        }
    }
    printf("%s %s, %d\n", Age_dict[m_age].fname, Age_dict[m_age].lname, m_age);

    //free everything in Age_dict
    for (unsigned int i = 0; i < MAX_AGE; i++) {
        if (Age_dict[i].fname) {
            free(Age_dict[i].fname);
            free(Age_dict[i].lname);
        }
    }

    return 0;
}